...
if __name__ == '__main__':
    ...
    os.system(f"cd data && python3 prepare_data_2d_custom.py -i {keypoint_2D_video_folder} -o myvideos --targeted-person-bboxes-path {targeted_person_bboxes_path} --custom-dataset-path {custom_dataset_path}")
    ...
    for idx, file in enumerate(os.listdir(mp4_video_folder)):
        ...
        os.system(f"python run.py --custom-dataset-path {custom_dataset_path} -d custom -k myvideos -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_detectron_coco.bin --render --viz-subject {file} --viz-action custom --viz-camera 0 --viz-video {os.path.join(mp4_video_folder,file)} --viz-export {os.path.join(keypoint_3D_video_folder,file)} --viz-size 6")
